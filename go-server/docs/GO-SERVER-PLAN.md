...
- G3 areas (`internal/world/areas.go`): 7 types — spawn, music, pvp, safe, door/trigger, camera, event; each `{id, x, y, w, h, kind, data}` from world.json; enter/exit hooks fire music/pvp-flag/teleport/dialog; areas checked on every movement Step + teleport.
...
- G9 social (`internal/social/`): friends (add/remove/block/list, online notify) + guilds (create/invite/accept/kick/leave/rank promote-demote, persist `guilds`/`guild_members`) + relay frame (guild/global chat routed via hub when `HUB_ENABLED`, else local broadcast).
